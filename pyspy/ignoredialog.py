...
class IgnoreDialog(wx.Frame):
    def __init__(self, parent, *args, **kwds):
        ...
        self.ignored_entities = config.OPTIONS_OBJECT.Get(
            "ignoredList", default=[])
        self._populateList()
    ...
    def __do_layout(self):
        ...
        instrLbl = wx.StaticText(
            self,
            wx.ID_ANY,
            "Select entities to be removed from ignore list:",
            style=wx.ALIGN_LEFT)
    ...
    def _populateList(self):
        ...
        if len(self.ignored_entities) > 1:
            self.ignored_entities = sortarray.sort_array(
                self.ignored_entities, 2, 1)
